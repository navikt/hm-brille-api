...
class KafkaRapid(
    ...
    /**
     * venter på ack med en timeout, og ved kafka feil propagerer opp exceptions.
     */
    fun <T> publishTimeOut(key: String?, event: T, timeOutSec: Long = 10) {
        try {
            check(!producerClosed.get()) { "can't publish messages when producer is closed" }
            val record = ProducerRecord(rapidTopic, key, mapper.writeValueAsString(event))
            producer.send(record).get(timeOutSec, TimeUnit.SECONDS)
        } catch (e: Exception) {
            log.error("Sending av event til '$rapidTopic' feilet")
            throw KafkaService.KafkaException("Sending av event til '$rapidTopic' feilet", e)
        }
    }
